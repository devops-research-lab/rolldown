...
///
/// `"type"` comes from the nearest manifest. A package root owns the `sideEffects` globs, so
/// inside `node_modules` the two can be different files.
...
impl<Fs: FileSystem> Resolver<Fs> {
  ...
  /// Collects both manifests in one walk, each following the rule `oxc_resolver` applies to the
  /// same file. The two rules differ inside `node_modules`, so a single manifest cannot serve
  /// both. See [`PackageScopes`].
