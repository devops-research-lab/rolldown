...
///
/// `oxc_resolver` reads `"type"` from the nearest scope, but only for `.js` and `.ts`. Rolldown
/// then falls back to the owning manifest for every js-like extension. The nearest manifest for
/// all four would invert the answer for `.jsx` and `.tsx`.
