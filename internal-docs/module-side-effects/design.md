...
tree-shaking may drop it. Three sources can supply it, in a fixed rank. For the
...
no `packageJsonPath`. The module then arrived with no package policy. The same file reached
through a relative import arrived with a policy. Two things went wrong at once:
