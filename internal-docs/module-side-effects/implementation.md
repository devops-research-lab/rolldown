...
`oxc_resolver` reads the nearest manifest, but only for `.js` and `.ts`. Rolldown then falls
...
Where they disagree, one file gets two answers, one per specifier. That is the bug this page
exists to prevent.
...
Two gaps remain. Neither is a regression, because both predate the recovery.
`oxc_resolver`'s type lookup crosses the `node_modules` and `@scope` boundaries, and this walk
